...
public class Problem3 extends BaseClass {
    ...
    private static void bePositive(Object[] arr, int arrayNumber) {
        ...
        for(int i = 0; i < arr.length; i++){ //iterate through array
             ...
             else if(arr[i] instanceof Double){ // turn it back to its original data type
             output[i] = Math.abs((Double) arr[i]); //use Math.abs to make values positive
            
             }

             else if(arr[i] instanceof Float){ // turn it back to its original data type
             output[i] = Math.abs((Float) arr[i]); //use Math.abs to make values positive
            
             }
             else if(arr[i] instanceof String){
                String string = (String) arr[i];                               // turn it back to its original data type
                output[i] = String.valueOf(Math.abs(Double.parseDouble(string))); //use Math.abs to make values positive
